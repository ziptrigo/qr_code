...
class QRCodeCreateSerializer(serializers.ModelSerializer):
    ...
    url = serializers.URLField(write_only=True, required=False, 
                               help_text="Original URL for QR code")
    ...
    class Meta:
        model = QRCode
        fields = [
            'id', 'qr_format', 'size', 'error_correction', 'border',
            'background_color', 'foreground_color', 'use_url_shortening',
            'url', 'data', 'created_at'
        ]
    ...
    def validate(self, data):
        ...
        if data.get('url') and data.get('data'):
            raise serializers.ValidationError(
                "Provide either 'url' or 'data', not both."
            )
        return data
...
class QRCodeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = QRCode
        fields = [
            'id', 'content', 'original_url', 'use_url_shortening',
            'short_code', 'qr_format', 'size', 'error_correction',
            'border', 'background_color', 'foreground_color',
            'image_file', 'image_url', 'redirect_url', 'scan_count',
    ...
    def get_image_url(self, obj):
        ...
        if request and obj.image_file:
            return request.build_absolute_uri(
                f"{settings.MEDIA_URL}{obj.image_file}"
            )
        return None

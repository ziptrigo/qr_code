...
def select_env(project_root: Path, environment_var: str | None = None) -> EnvSelection:
    ...
    raw_env = (environment_var if environment_var is not None else os.getenv('ENVIRONMENT', '')).strip()
    ...
    if environment:
        if environment not in SUPPORTED_ENVIRONMENTS:
            ...
            return EnvSelection(environment=raw_env, env_path=None, errors=errors, warnings=warnings)

        env_path = project_root / f'.env.{environment}'
        if not env_path.exists():
            errors.append(f'Environment file `{env_path}` not found.')
            return EnvSelection(environment=environment, env_path=None, errors=errors, warnings=warnings)
